...
The `LineaRollup.ts`specifies the finalised L2 block number as the context for virtual machine requests (returned by calling `currentL2BlockNumber` on the Linea rollup contract).
...
We interface with our port of this library through our `LineaVerifier.sol` that is deployed on L1.
