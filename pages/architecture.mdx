...
- Internally this method implements the [ERC-3668 specification](https://eips.ethereum.org/EIPS/eip-3668) and reverts with an `OffchainLookup` telling the client to query our `Gateway`. 
...
An `EVMRequest` is made up of a series of operations (`ops`) that are interpreted by the [virtual machine](/virtual-machine) to read and manipulate data stored on other blockchains. 
...
We also offer a [Typescript](https://www.typescriptlang.org/) implementation of the request builder and virtual machine to allow for easy development and testing of requests. Take a look at our [Examples](/examples) page for some.. examples.
...
The gateway interfaces with the appropriate blockchain to source data, generate proofs and return them to the interfacing client. This removes the need for users or clients to have to directly interface the chains in question.
