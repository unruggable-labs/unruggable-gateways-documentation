...
Our stack-based virtual machine is designed to read data from EVM-compatible blockchains and generate proofs to verify that the data is correct.
...
A virtual machine request (`EVMRequest`) specifies how many outputs it expects, and how those outputs should be fetched.
...
In the Typescript version, this state is managed by the `MachineState` class in `vm.ts`. In the Solidity version, it is represented by the `Machine` struct in `EVMProver.sol`.